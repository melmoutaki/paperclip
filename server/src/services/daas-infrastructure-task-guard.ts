  ...
  },
  {
    ...
    patterns: [
      ...
      /\b(run|execute)\s+(a\s+)?(bash|sh)\s+(command|script)\s+on\b/i,
	      ...
	      /\b(run|execute)\b[^.\n]{0,80}\b(ls|whoami|cat|tail|grep|awk|sed|ps|df|du|free|top|uptime|journalctl|systemctl|docker|podman|kubectl)\b[^.\n]{0,80}\b(on|in|from)\b[^.\n]{0,50}\b((\d{1,3}\.){3}\d{1,3}(\/\d{1,2})?|\[[0-9a-f:]{3,}\]|[a-z0-9][a-z0-9-]*(\.[a-z0-9][a-z0-9-]*)+)\b/i,
	      /\b(tail|view|read|show|inspect|check)\b[^.\n]{0,80}\b(logs?|disk|env|environment|vars?|variables?)\b[^.\n]{0,80}\b(on|in|from)\b[^.\n]{0,50}\b((\d{1,3}\.){3}\d{1,3}(\/\d{1,2})?|\[[0-9a-f:]{3,}\]|[a-z0-9][a-z0-9-]*(\.[a-z0-9][a-z0-9-]*)+)\b/i,
  ...
  },
  {
    ...
    patterns: [
      ...
      /\b(aws|gcloud|az)\b[^.\n]{0,80}\b(reboot|restart|start|stop|terminate)\b[^.\n]{0,80}\b(ec2|compute|vm|instance|server)\b/i,
	      ...
	      /\b(reboot|restart|stop|start)\b[^.\n]{0,40}\b(server|host|vm|node|instance)\b/i,
	      /\b(reboot|restart|reload|stop|start)\b[^.\n]{0,60}\b(nginx|apache|postgres|postgresql|mysql|redis|systemd|service|web-\d+|db-\d+|api-\d+|worker-\d+)\b/i,
	      /\b(nginx|apache|postgres|postgresql|mysql|redis|systemd|service|web-\d+|db-\d+|api-\d+|worker-\d+)\b[^.\n]{0,60}\b(reboot|restart|reload|stop|start)\b/i,
	      /\b(reboot|restart|reload|stop|start|rotate|renew|replace|deploy|migrate|backup|restore)\b[^.\n]{0,80}\b(nginx|apache|postgres|mysql|redis|database|db|service|systemd|tls|ssl|certificate|cert)?\b[^.\n]{0,80}\b(on|in|for|from|to)\b[^.\n]{0,50}\b((\d{1,3}\.){3}\d{1,3}(\/\d{1,2})?|\[[0-9a-f:]{3,}\]|[a-z0-9][a-z0-9-]*(\.[a-z0-9][a-z0-9-]*)+)\b/i,
	      ...
	      /\b(rotate|renew|replace)\b[^.\n]{0,70}\b(tls|ssl|certificate|cert|database\s+password|db\s+password|root\s+password|admin\s+password)\b/i,
	      /\b(run|execute|apply)\b[^.\n]{0,60}\b(database\s+|db\s+)?migration\b/i,
